...
species = sys.argv[1]
fold_nr = int(sys.argv[2])


if len(sys.argv) == 3:
	missingFraction = 0.0
else:
	missingFraction = float(sys.argv[3])
...
for fold, (train, test) in enumerate(cv.split(Y)):
	...
	with open('../res/' + species + '/missing' + str(int(100 * missingFraction)) + '/perprotein/'+ str(fold) + '.pkl', 'w') as fw:
		pickle.dump([auc, fmax, nsmin], fw)
